...
class HelpCommand(commands.Cog):
    ...
    async def helpcommand(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="`/middleman-embed`", value="*Send embed to start middleMan Ticket with multi-crypto support*", inline=False)
        embed.add_field(name="`/ping`", value="*Reply with pong !*", inline=False)
        embed.add_field(name="`/release`", value="*Developer only: Release funds from stuck wallet*", inline=False)
        embed.add_field(name="`/setup-category`", value="*Setup category where ticket are opened*", inline=False)
        embed.add_field(name="`/setup-developer-role`", value="*Setup developer role for /release command*", inline=False)
